...
CVgroup5 <- function(k,datasize){
  ...
  return(cvlist)
  #meigeyangbenchadebudao1meiyoujiaquanpingjunshudebiyaokeyishuo
}

CVgroup9 <- function(k,datasize){
  cvlist <- list()
  set.seed(9)
  n <- rep(1:k,ceiling(datasize/k))[1:datasize]
  temp <- sample(n,datasize)
...
for (inum in 1:20) {
	...
	for (i in types) {
		...
		if (positivenum >= 10) {
			...
			if (inum <= 10) {
				posigroup <- CVgroup5(10,positivenum)
				positest = posigroup[[inum]]
			}else{
				posigroup <- CVgroup9(10,positivenum)
				positest = posigroup[[(inum-10)]]
			}
