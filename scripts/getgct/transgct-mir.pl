...
while (<REF>) {
	chomp;
	my ($fileid, $detailfile, $sampleid) = (split(/\t/, $_))[0,1,6];
	...
	if (exists($hash{$sampleid})) {
		...
		my $previousfileid = $hashid{$sampleid};

		unless ($ref{$previousfileid} =~ m/--/) {
			$ref{$previousfileid} = $ref{$previousfileid}."--$previousfile";
		}
		$ref{$fileid} = $sampleid."--$detailfile";
	}else{
		$hash{$sampleid} = $detailfile;
		$hashid{$sampleid} = $fileid;
		$ref{$fileid} = $sampleid;
	}
	$refc{$fileid} += 1;
...
for (my $var = 0; $var < $genelength; $var++) {
	...
	my $array = join("\t", @array);
	#if ( ($array[0] =~ m/^hsa-/) or ($array[0] =~ m/^Name/) ) {
		print OUT "$array\n";
	#}
