...
for (i in 1:alllen) {
	...
	print(a)
   	...
   	if (length(unique(a)) < 0.8*length(a)) {
   		next
    }
    if (length(unique(b)) < 0.8*length(a)) {
   		next
    }
    #another standard 20.
   	mirtruenum = sum(a != 0)
	...
	res = t(c(rownames[i], length(unique(a)), mean(a), length(unique(b)), mean(b), norcor, specor, kencor, norcor2, specor2, kencor2, norcor3, speco3, kencor3, norcor4, speco4, kencor4, norcor5, speco5, kencor5, (bunderbc + bundercb)/2, (cunderbc + cundercb)/2, norcor^2, norcor2^2))
...
#		cundercb = rescb[1] /(rescb[1] + rescb[2])
#
#		res = t(c(rownames[i], length(a), mean(a), length(b), mean(b), norcor, specor, kencor, norcor2, specor2, kencor2, norcor3, speco3, kencor3, norcor4, speco4, kencor4, norcor5, speco5, kencor5, (bunderbc + bundercb)/2, (cunderbc + cundercb)/2, norcor^2, norcor2^2))
